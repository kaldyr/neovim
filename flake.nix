{
    ...
    let
        ...
        runtimePaths = pkgs.lib.strings.concatStrings (
            [ "${./config}," "${grammarsPath}," "${pkgs.neovimPlugins.lazy-nvim}," ]
            ++
            pkgs.lib.lists.forEach nixpkgPlugins ( plugin: "${plugin}," )
            ++
        ...
        pluginPathsLua = builtins.concatStringsSep "\n" (
            pkgs.lib.lists.forEach nixpkgPlugins
                ( plugin: "PluginsFromNix['${plugin.pname}'] = '${plugin}'" )
            ++
            pkgs.lib.attrsets.mapAttrsToList
                ( name: outPath: "PluginsFromNix['${name}'] = '${outPath}'" )
                pkgs.neovimPlugins
        );
